...
def test_create_summary(test_app: TestClient, monkeypatch: MonkeyPatch):
...
def test_read_summary(test_app: TestClient, monkeypatch: MonkeyPatch):
    test_data = {
        "id": 1,
        "url": "https://foo.bar",
        "summary": "summary",
...
def test_read_summary_incorrect_id(test_app: TestClient, monkeypatch: MonkeyPatch):
    async def mock_get(id):
        return None

    monkeypatch.setattr(crud, "get", mock_get)
    response = test_app.get("/summaries/10000/")

    assert response.status_code == 404
    assert response.json()["detail"] == "Summary not found"


def test_read_all_summaries(test_app: TestClient, monkeypatch: MonkeyPatch):
    test_data = [
        {
            "id": 1,
            "url": "https://foo.bar",
...
def test_remove_summary(test_app: TestClient, monkeypatch: MonkeyPatch):
    async def mock_get(id):
        return {
            "id": 1,
            "url": "https://foo.bar",
...
def test_remove_summary_incorrect_id(test_app: TestClient, monkeypatch: MonkeyPatch):
    async def mock_get(id):
        return None

    monkeypatch.setattr(crud, "get", mock_get)

    response = test_app.delete("/summaries/10000/")

    assert response.status_code == 404  # not found status expected
    assert response.json()["detail"] == "Summary not found"


def test_update_summary(test_app: TestClient, monkeypatch: MonkeyPatch):
    ...
    async def mock_put(id, updated_summary):
